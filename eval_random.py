...
def play_against_random(agent: Agent, verbose: bool = False):
    ...
    for _ in range(200):
        ...
        if verbose:
            print(f"{current_player} did action: {move}")
            print(state[-1])
...
mode = 7 if len(sys.argv) > 1 and sys.argv[1] == "7" else 5
model_path = "./test_models/BestModel_7x7" if len(sys.argv) > 1 and sys.argv[1] == "7" else "./test_models/model_5x5"

best_agent = Agent(BLACK, dimension=mode).load(model_path)
